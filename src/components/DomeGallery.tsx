...
function buildItems(pool: ImageItem[], seg: number, projects?: CaseStudy[]): ItemDef[] {
  ...
  if (projects && projects.length > 0) {
    ...
    for (let i = 1; i < usedStudies.length; i++) {
      if (usedStudies[i].image === usedStudies[i - 1].image) {
        for (let j = i + 1; j < usedStudies.length; j++) {
          if (usedStudies[j].image !== usedStudies[i].image) {
            const tmp = usedStudies[i];
            usedStudies[i] = usedStudies[j];
            usedStudies[j] = tmp;
            break;
          }
        }
...
export default function DomeGallery({
  ...
  const cssStyles = `
    ...
    @media (min-width: 768px) {
      .dg-card { flex-direction: row; }
    }
    .dg-card-img-container {
      flex: 1.2;
      position: relative;
      background: #f3f4f6;
      height: 40%;
    ...
    @media (min-width: 768px) {
      .dg-card-img-container { height: 100%; }
    }
    .dg-card-img-container img {
      width: 100%;
      height: 100%;
      object-fit: cover;
    }
    .dg-card-content-container {
      flex: 1;
      padding: 40px;
      display: flex;
      flex-direction: column;
    ...
    .dg-btn-close:hover { color: black; }
  `;
              ...
              {items.map((it, i) => (
                ...
                >
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    aria-label={it.alt || 'Open project'}
                    onClick={e => {
